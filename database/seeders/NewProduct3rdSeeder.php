...
class NewProduct3rdSeeder extends Seeder
{
    /**
    ...
    public function data()
    {
        ...
        if (is_array($responses)) {
            ...
            foreach ($responses as $key_product => $product) {
                ...
                if ($category) {
                    ...
                    if (strpos(strtolower($product->title), 'backpack') !== false) {
                        $new_category = Category::where('slug', 'bags')->first();
                        $merge['category_id'] = $new_category->id;
                    } else if (strpos(strtolower($product->category), 'electronics') !== false) {
                        $new_category = Category::where('slug', 'electronic')->first();
                        $merge['category_id'] = $new_category->id;
                    } else if (strpos(strtolower($product->title), 'jacket') !== false) {
                        $new_category = Category::where('slug', 'jacket')->first();
                        $merge['category_id'] = $new_category->id;
                    } else {    
                ...
                if (str_contains_all(strtolower($product->title), ['shirt', 'dress', 't-shirts'])) {
                    $merge = [
                        'stock' => [
                            [
                                'name' => 'S',
