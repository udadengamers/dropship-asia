    ...
    <section class="card">
        <div class="card-body">
            {{-- <form action="{{ route('administrator.product.update', $product->uuid) }}" method="post" enctype="multipart/form-data"> --}}
                @method('put')
                @csrf
                @include('administrator.product.parts.form_show', $product)
    ...
    @push('after-scripts')
        <script>
            ...
            function dynamic_field(number)
            {
                ...
                html += '<td><input type="number" name="stock['+number+'][quantity]" class="form-control variation-quantity" required/></td>';
                html += '<td><input type="number" name="stock['+number+'][price]" class="form-control variation-price" required/></td>';

                if (number > 1) {
                    html += '<td><button type="button" disabled name="remove" id="" class="btn btn-danger remove">Remove</button></td></tr>';
                    $('tbody').append(html);
                }
            }
            ...
            $(document).on('click', '.remove', function(){
                ...
                body = $('#variationTbody tr');
                if (body.length < 1) {
                    $("#quantity").prop({"type": "number", "value": '', "readonly": false});
                    $("#price").prop({"type": "number", "value": '', "readonly": false});
                }
            });
